...
import type { Problem, ProblemSolveStatusAfterFailingSystests, ProblemSolveStatusAfterPassingSystests } from "../problems/types";
  ...
  return contestUsersData.map(contestUserData => {
    ...
    return {
      ...
      problemSolveStatuses: declareRecordByInitializer(
        ProblemPlacements,
        (placement) => {
          const problemPosition = computeProblemPositionFromProblemPlacement(placement);
          const problemPretestsQuality = problems[problemPosition].pretestsQuality;
          const problemSolveStatus = problemSolveStatuses[placement];
          const submissions = problemSolveStatus.submissions;
          submissions.reverse();
          const lastPassingSubmissionIndex = submissions.findIndex(submission => submission.verdict === "Pretests passed");

          if (problemSolveStatus.phase === "after-passing-pretests" && Math.random() < problemPretestsQuality) {
